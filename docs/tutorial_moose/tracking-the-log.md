...
Now we can test whether our code is working! Firstly, delete the existing `simvue_thermal.txt` file, so that the FileMonitor doesn't add logs from the previous run to our next run. In reality, you would want to create new directories to contain logs and results for each simulation you perform to avoid this issue, but for testing purposes we don't need to keep any of our old logs or results. 
...
## Adding Metadata
To allow us to keep track of how a particular run has been performed, we may want to add metadata to the Simvue run. As an example, in the console log, important information about the MOOSE environment and simulation parameters are included at the very top. This can be seen by running `head simvue_thermal.txt -n 40`:
```

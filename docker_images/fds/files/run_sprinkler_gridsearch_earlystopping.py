...
input_file = f90nml.read("/workdir/sprinkler_earlystopping.fds")
...
for activation_temperature in range(60, 101, 40):
        for flow_rate in range(120, 201, 80):
                for droplet_diameter in range(300, 701, 400):
                        ...
                        f90nml.patch("/workdir/sprinkler_earlystopping.fds", patch, "/workdir/sprinklers_patched.fds")
                        ...
                        with FDSRun() as run:
                            ...
                            run.init(f"fds_simulation_sprinklers_{counter}",folder="/fds/sprinklers-gridsearch", tags=["fds", "sprinkler", "gridsearch", "boundaries", "earlystopping"])
